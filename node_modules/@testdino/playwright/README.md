...
| Option      | CLI Flag                               | Environment Variable  | Config / reporter  | Description                                                                                                    |
| ----------- | -------------------------------------- | --------------------- | ------------------ | -------------------------------------------------------------------------------------------------------------- |
| `token`     | `--token`, `-t`                        | `TESTDINO_TOKEN`      | `token`            | Authentication token (required)                                                                                |
| `serverUrl` | `--server-url`                         | `TESTDINO_SERVER_URL` | `serverUrl`        | TestDino server URL                                                                                            |
| `debug`     | `--debug`                              | `TESTDINO_DEBUG`      | `debug`            | TestDino debug logging (not Playwright Inspector)                                                              |
| `ciRunId`   | `--ci-run-id`                          | `TESTDINO_CI_RUN_ID`  | `ciRunId`          | Group sharded test runs                                                                                        |
| `splitId`   | `--split-id`                           | `TESTDINO_SPLIT_ID`   | `splitId`          | Group ID for split mode                                                                                        |
| `split`     | `--split <c/t>`                        | -                     | -                  | Split position. CLI-only                                                                                       |
| `artifacts` | `--no-artifacts`                       | -                     | `artifacts: false` | Disable artifact uploads                                                                                       |
| `coverage`  | `--coverage`                           | -                     | `coverage.enabled` | Enable code coverage collection                                                                                |
| `tags`      | `--tags <csv>`                         | `TESTDINO_TAGS`       | `tags`             | Run-level labels. `tdpw` flag; Playwright rejects `--tags`                                                     |
| —           | `--rerun <scope>` `--from-run <runId>` | —                     | —                  | Re-run a run's failed/flaky tests. CLI-only; see [Re-run only the failed tests](#re-run-only-the-failed-tests) |
...
`tdpw test` wraps Playwright when you would rather not edit your config, or need a run-scoped flag like `--ci-run-id` or `--tags`. Any option TestDino does not recognize is passed through to Playwright:
...
npx tdpw test --tags regression,smoke
```

`--tags` is a `tdpw` flag. `npx playwright test --tags ...` exits with `error: unknown option '--tags'` because Playwright does not define that option. To label a run while keeping `npx playwright test`, set `TESTDINO_TAGS`, `tags` in `testdino.config.ts`, or `tags` in the reporter options.
...
> **Note:** a `--reporter` after `--` is merged with the TestDino reporter (`--reporter=line` becomes `--reporter=line,@testdino/playwright`), so your reporter and the upload both run. Before 2.7.0 the user reporter replaced TestDino's and the run was never reported.
...
- **Run each slot exactly once per `--split-id`.** `--split` is manual: TestDino does not slice your tests — you decide what each slot runs (files, `--grep`, `--project`). Re-running a failed slot is fine. Do **not** send two runs through the same slots under one split-id (for example `--project dnp` and `--project documents` both as `--split 1/3 --split-id $BUILD`): the second process for a slot is **refused** — its results are not recorded, the reporter prints the reason (`SPLIT_SLOT_DUPLICATE`), and the dashboard shows the same notice. To report several test sets, give each its own split-id (`--split-id $BUILD-dnp`, `--split-id $BUILD-documents`), or shard inside the slot (`--split 1/3 -- --shard=1/2`).

### Re-run only the failed tests

A finished run already knows which of its tests failed. Instead of naming them, ask TestDino for the selection and run just those:

```bash
npx tdpw test --rerun failed --from-run <runId>
```

`<runId>` is the run's id from its TestDino URL (`test_run_…`). The CLI resolves the selection with the same pipeline key it reports with, hands it to Playwright as `--test-list`, and reports the re-run as an ordinary run linked to its source.

- `--rerun <scope>` — `failed` (failed or timed out), `flaky` (passed only on a retry), or `failed-and-flaky`. Required with `--from-run`.
- `--test-ids a,b` — exactly these Playwright test ids instead of the scope; `--exclude-ids c` drops ids from the scope. The dashboard's re-run sheet writes these for you.
- Needs **Playwright 1.56 or newer** (`--test-list`). On older releases the CLI refuses rather than running the whole suite. Requires `@testdino/playwright` **2.7.0 or newer** — an older CLI forwards the flag to Playwright, which fails with `error: unknown option '--rerun'`.
- Cannot be combined with `--shard`, `--split`, `--grep`, `--grep-invert`, `--last-failed` or `--test-list`: the selection already decides what runs. `--project` and file filters after `--` still apply and can exclude selected tests; the CLI refuses when nothing is left. Put the re-run flags **before** `--`; everything after it goes to Playwright unchanged.
- Before running, the CLI checks the selection against your checkout. Tests renamed or deleted since the source run are skipped with a warning; if **none** match, it exits 1 rather than reporting a green run that ran nothing. A title containing `" › "` cannot be selected by title (Playwright's list format has no escape) and is named so you can run it by hand.
- If the source run finished seconds ago, the CLI waits up to 90 s for its results to be processed.

To dispatch a re-run from the TestDino dashboard, your workflow declares `workflow_dispatch` inputs the dashboard fills in; see the TestDino docs for the snippet.
...
**Two exceptions** surface as a banner and a non-zero exit so CI can catch them: events dropped after the reporter has exhausted its retries (partial-sync), and a quota error hit mid-run that leaves data missing on TestDino. Both are business-visible signals rather than reporter-infra failures.

Distinct from those: if your organization is **already over its execution limit** when a run starts, the reporter records the run as blocked, prints an advisory notice, and **leaves your build status unchanged** — the tests run to completion and their results simply aren't sent to TestDino. This is not a failure and does not affect CI exit.
...
| `TestDino Execution Limit Reached`            | A quota error was hit mid-run: tests still run, CI exit is non-zero. Upgrade plan or wait for quota reset                                                             |
| `TestDino: this run was not recorded`         | Your org was already over its execution limit at run start: tests still run, **CI exit is unchanged** (Playwright's own result). Upgrade plan or wait for quota reset |
...
| `error: unknown option '--tags'`              | `--tags` is a `tdpw` flag. Use `npx tdpw test --tags ...` or `TESTDINO_TAGS` with `npx playwright test`                                                               |
