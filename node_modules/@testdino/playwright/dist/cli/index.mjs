...
function sanitizeRule(raw) {
  ...
  if (typeof data.action !== "string" || !VALID_ACTIONS.has(data.action)) return null;
  if (!Array.isArray(data.conditions) || data.conditions.length === 0) return null;
  const tagValue = typeof data.tagValue === "string" && data.tagValue.length > 0 ? data.tagValue : void 0;
  if (data.action === "tag" && !tagValue) return null;
  const conditions = [];
  for (const candidate of data.conditions) {
    const condition = sanitizeCondition(candidate);
...
function isPreTestMatchable(rule) {
  return rule.conditions.every((condition) => condition.field !== "errorMessage");
}
...
function matchesSome(resolved, condition) {
  if (!resolved || condition.values.length === 0) return false;
  return condition.values.some((value) => resolved.some((element) => valueMatches(element, value, condition.isRegex)));
}
function matchesEvery(resolved, condition) {
  if (!resolved || condition.values.length === 0) return false;
  return condition.values.every((value) => resolved.some((element) => valueMatches(element, value, condition.isRegex)));
}
...
var POSITIVE_OPERATORS = /* @__PURE__ */ new Set(["is", "isAny", "in", "includes", "includesSome"]);
...
function compileSkipInvertPattern(rules, runCtx) {
  ...
  for (const rule of rules) {
    if (rule.action !== "skip" || !isPreTestMatchable(rule) || rule.conditions.length === 0) continue;
    const pattern = compileRule(rule, runCtx);
    if (pattern !== void 0) rulePatterns.push(pattern);
  }
  if (rulePatterns.length === 0) return void 0;
  if (rulePatterns.length === 1) return rulePatterns[0];
  return rulePatterns.map((p) => `(?:${p})`).join("|");
}
function compileRule(rule, runCtx) {
  const fragments = [];
  for (const condition of rule.conditions) {
    if (GIT_FIELDS.has(condition.field)) {
      if (!matchCondition(condition, { git: runCtx })) return void 0;
      continue;
    }
    if (!GREP_FIELDS.has(condition.field) || !POSITIVE_OPERATORS.has(condition.operator)) return void 0;
    const conditionFragments = compileCondition(condition);
    if (conditionFragments === void 0) return void 0;
...
function compileCondition(condition) {
  ...
  const anchored = condition.field === "titlePath" && condition.operator === "is";
  switch (condition.operator) {
    case "is":
      return [{ source: pieces[0], anchored }];
    case "isAny":
    case "in":
    case "includesSome":
      return [{ source: pieces.length === 1 ? pieces[0] : `(?:${pieces.join("|")})`, anchored: false }];
    case "includes":
      return pieces.map((source) => ({ source, anchored: false }));
    default:
      return void 0;
  }
}
function compileValuePattern(value, isRegex) {
  if (!isRegex) return escapeRegex(value);
...
function combineFragments(fragments) {
  ...
  if (fragments.length === 1) {
    const f = fragments[0];
    return f.anchored ? `^${f.source}` : f.source;
  }
  return `^${fragments.map((f) => f.anchored ? `(?=${f.source})` : `(?=.*${f.source})`).join("")}`;
}
