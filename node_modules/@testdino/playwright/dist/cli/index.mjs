...
function sanitizeCondition(data) {
  ...
  if (!Array.isArray(data.values) || !data.values.every((v) => typeof v === "string")) return null;
  return {
    field: data.field,
...
function matchesSingle(resolved, condition) {
  ...
  return resolved.some((element) => valueMatches(element, value, condition.isRegex));
}
...
function someValueMatched(elements, condition) {
  return condition.values.some((value) => elements.some((element) => valueMatches(element, value, condition.isRegex)));
}
function everyValueMatched(elements, condition) {
  return condition.values.every((value) => elements.some((element) => valueMatches(element, value, condition.isRegex)));
}
function valueMatches(element, value, isRegex) {
  if (!isRegex) return element === value;
  if (value.length > MAX_REGEX_PATTERN_LEN) return false;
...
function compileSkipInvertPattern(rules, runCtx) {
  const rulePatterns = [];
  for (const rule of rules) {
    if (!ruleHasAction(rule, "skip") || !isPreTestMatchable(rule)) continue;
    const compiled = compileNode(rule.conditions, runCtx);
    if (compiled === void 0) continue;
    if (compiled.kind === "const") {
      if (compiled.value) rulePatterns.push(".*");
      continue;
    }
    rulePatterns.push(`^${compiled.source}`);
  }
  if (rulePatterns.length === 0) return void 0;
  if (rulePatterns.length === 1) return rulePatterns[0];
  return rulePatterns.map((p) => `(?:${p})`).join("|");
}
function compileNode(node, runCtx) {
  if (!isConditionGroup(node)) return compileLeaf(node, runCtx);
  if (node.cond.length === 0) return void 0;
  const asserts = [];
  for (const child of node.cond) {
    const compiled = compileNode(child, runCtx);
...
function compileLeaf(condition, runCtx) {
  ...
  for (const value of condition.values) {
    const piece = compileValuePattern(value, condition.isRegex);
...
function compileValuePattern(value, isRegex) {
  if (!isRegex) {
    const escaped = escapeRegex(value).replace(/\//g, "[/\\\\]");
    return `(?:^| )${escaped}(?= |$)`;
  }
  if (value.length > MAX_REGEX_PATTERN_LEN) return void 0;
  try {
    new RegExp(value);
  } catch {
    return void 0;
...
var TestCommand = class {
  ...
  async execute(options, args) {
    ...
    try {
      ...
      if (mergedConfig.token) {
        ...
        this.logger.info(
          `TestDino Actions: ${mergedConfig.actionRules.rules.length} rule(s) fetched`
        );
      }
      ...
      if (mergedConfig.actionRules) {
        const skipPattern = compileSkipInvertPattern(mergedConfig.actionRules.rules, mergedConfig.runGitContext ?? {});
        this.logger.info(
          skipPattern ? "TestDino Actions: skip filter active (compiled into --grep-invert)" : "TestDino Actions: no compilable skip rules for this run"
        );
        if (skipPattern) {
          ...
          const skipsEverything = skipPattern === ".*" || skipPattern.includes("(?:.*)");
  ...
  async collectRunGitContext() {
    ...
    if (metadata.commit?.message) git.message = metadata.commit.message;
