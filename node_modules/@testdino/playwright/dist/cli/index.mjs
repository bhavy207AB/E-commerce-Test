...
var RerunSplitConflictError = class extends TestDinoError {
  constructor(source) {
    const message = `A re-run cannot be combined with split mode (${source})

A re-run is a single run of the selected tests. Drop the split flags:
  \u2022 npx tdpw test --rerun failed --from-run <runId>`;
    super(message);
    this.name = "RerunSplitConflictError";
  }
};
...
async function awaitUpdateResult(pending) {
  let timer;
  const grace = new Promise((resolve2) => {
    timer = setTimeout(() => resolve2(void 0), SETTLE_GRACE_MS);
    timer.unref?.();
  });
  try {
...
var TESTDINO_REPORTER = "@testdino/playwright";
function withTestdinoReporter(args) {
  const merged = [];
  let userReporter = false;
  for (let i = 0; i < args.length; i++) {
    const arg = args[i];
    if (arg === "--reporter" && i + 1 < args.length) {
      merged.push(`--reporter=${args[++i]},${TESTDINO_REPORTER}`);
      userReporter = true;
    } else if (arg.startsWith("--reporter=")) {
      merged.push(`${arg},${TESTDINO_REPORTER}`);
      userReporter = true;
    } else {
      merged.push(arg);
    }
  }
  return userReporter ? merged : ["--reporter", TESTDINO_REPORTER, ...merged];
}
var PlaywrightSpawner = class {
  ...
  async spawn(options) {
    ...
    try {
      ...
      const playwrightArgs = ["playwright", "test", ...withTestdinoReporter(args)];
...
// src/cli/rerun/client.ts
var NOT_READY_BUDGET_MS = 9e4;
var NOT_READY_POLL_MS = 2e3;
var REQUEST_TIMEOUT_MS = 15e3;
var RerunSelectionError = class extends Error {
  constructor(message, code) {
    super(message);
    this.code = code;
    this.name = "RerunSelectionError";
  }
  code;
};
var defaultSleep = (ms) => new Promise((resolve2) => setTimeout(resolve2, ms));
function rerunSelectionPath(runId) {
  return `/api/v1/reporter/runs/${encodeURIComponent(runId)}/rerun-selection`;
}
async function fetchRerunSelection(options) {
  const { fetchImpl = fetch, sleep = defaultSleep, now = Date.now } = options;
  const url = `${options.serverUrl.replace(/\/+$/, "")}${rerunSelectionPath(options.runId)}`;
  const body = JSON.stringify({
    ...options.testIds?.length ? { testIds: options.testIds } : { scope: options.scope },
    ...options.excludeTestIds?.length ? { excludeTestIds: options.excludeTestIds } : {}
  });
  const deadline = now() + NOT_READY_BUDGET_MS;
  for (; ; ) {
    const response = await fetchImpl(url, {
      ...
      throw new RerunSelectionError(
        `Could not reach ${url}: ${error instanceof Error ? error.message : String(error)}`
      );
    });
    if (response.ok) {
      return await readSelection(response, url);
    }
    const code = await readErrorCode(response);
    if (code === "SELECTION_NOT_READY" && now() < deadline) {
      await sleep(NOT_READY_POLL_MS);
      continue;
    }
    throw new RerunSelectionError(describeFailure(response.status, code, options.runId), code);
  }
}
async function readSelection(response, url) {
  let body;
  try {
  ...
  if (!body || !Array.isArray(body.testList) || typeof body.resolution?.selected !== "number") {
    throw new RerunSelectionError(`Unexpected response from ${url}: not a re-run selection.`);
  }
  return body;
}
...
function describeFailure(status, code, runId) {
  switch (code) {
    case "RUN_NOT_FOUND":
      return `Run ${runId} is unknown, belongs to another project, or is not accessible with this token.`;
    case "RUN_NOT_FINALIZED":
      return `Run ${runId} has not finished yet, so its failed set is incomplete.`;
    case "SELECTION_NOT_READY":
      return `Run ${runId} finished but its results were never processed within ${NOT_READY_BUDGET_MS / 1e3}s. Check the run in TestDino before retrying.`;
    case "INVALID_PARAM":
      return `The re-run selection request for ${runId} was rejected as invalid.`;
    default:
      return `Could not resolve the re-run selection for ${runId} (HTTP ${status}${code ? ` ${code}` : ""}).`;
  }
}

// src/cli/rerun/prepare.ts
import { readFileSync as readFileSync2, unlinkSync as unlinkSync2, writeFileSync as writeFileSync3 } from "fs";
import { tmpdir as tmpdir3 } from "os";
import { dirname as dirname4, join as join5, resolve } from "path";
import { randomUUID as randomUUID3 } from "crypto";
var MIN_TEST_LIST_VERSION = [1, 56, 0];
var RERUN_FLAGS = ["--rerun", "--from-run", "--test-ids", "--exclude-ids"];
var CONFLICTING_FLAGS = ["--test-list", "--test-list-invert", "--last-failed", "--grep", "-g", "--grep-invert"];
var SHARD_FLAG = "--shard";
function hasFlag(args, flags) {
  return args.find((arg) => flags.some((flag) => arg === flag || arg.startsWith(`${flag}=`)));
}
function misplacedRerunFlag(playwrightArgs) {
  return hasFlag(playwrightArgs, RERUN_FLAGS);
}
function conflictingSelector(playwrightArgs) {
  return hasFlag(playwrightArgs, CONFLICTING_FLAGS);
}
function hasShardFlag(playwrightArgs) {
  return hasFlag(playwrightArgs, [SHARD_FLAG]) !== void 0;
}
function parseVersion(raw) {
  const match = raw ? /^(\d+)\.(\d+)\.(\d+)/.exec(raw.trim()) : null;
  return match ? [Number(match[1]), Number(match[2]), Number(match[3])] : void 0;
}
function supportsTestList(raw) {
  const version = parseVersion(raw);
  if (!version) return false;
  for (let i = 0; i < MIN_TEST_LIST_VERSION.length; i++) {
    if (version[i] > MIN_TEST_LIST_VERSION[i]) return true;
    if (version[i] < MIN_TEST_LIST_VERSION[i]) return false;
  }
  return true;
}
function readInstalledPlaywrightVersion(cwd = process.cwd()) {
  let dir = resolve(cwd);
  for (; ; ) {
    try {
      const parsed = JSON.parse(
        readFileSync2(join5(dir, "node_modules", "@playwright", "test", "package.json"), "utf-8")
      );
...
var SelectionFile = class {
  path;
  write(lines) {
    this.path = join5(tmpdir3(), `testdino-rerun-${randomUUID3()}.txt`);
    writeFileSync3(this.path, `${lines.join("\n")}
`, "utf-8");
    return this.path;
  }
  cleanup() {
...
// src/cli/rerun/verify.ts
import { execa as execa2 } from "execa";
var LIST_TIMEOUT_MS = 6e4;
var defaultListRunner = async (args) => {
  const result = await execa2("npx", ["playwright", "test", ...args, "--list", "--reporter=json"], {
...
function countReportTests(report) {
...
async function countSelectedTests(selectionPath, playwrightArgs, listRunner = defaultListRunner) {
  let report;
  try {
    report = JSON.parse(await listRunner([...playwrightArgs, `--test-list=${selectionPath}`]));
  } catch (error) {
    return { matched: 0, checked: false, reason: error instanceof Error ? error.message : String(error) };
  }
  const loadError = report.errors?.[0]?.message;
  if (loadError) {
    throw new TestDinoError(`Playwright could not load the selected tests:
${loadError}`);
  }
  return { matched: countReportTests(report), checked: true };
}

// src/cli/rerun/types.ts
var RERUN_SCOPES = ["failed", "flaky", "failed-and-flaky"];
function isRerunScope(value) {
  return RERUN_SCOPES.includes(value);
}
...
var SCOPE_LABELS = {
  failed: "failed",
  flaky: "flaky",
  "failed-and-flaky": "failed and flaky"
};
var TestCommand = class {
  ...
  async execute(options, args) {
    let tempConfigPath;
    const selectionFile = new SelectionFile();
    try {
      ...
      const misplaced = misplacedRerunFlag(filteredArgs);
      if (misplaced) {
        throw new TestDinoError(
          `${misplaced} must come before "--": everything after it goes to Playwright unchanged.

Use: npx tdpw test --rerun failed --from-run <runId> -- <playwright args>`
        );
      }
      if (options.rerun !== void 0) {
        const selection = await this.resolveRerunSelection(options, mergedConfig, filteredArgs);
        if (selection === null) {
          return { exitCode: 0, success: true };
        }
        const selectionPath = selectionFile.write(selection.testList);
        await this.verifyRerunSelection(selectionPath, filteredArgs, selection);
        filteredArgs = [...filteredArgs, `--test-list=${selectionPath}`];
        mergedConfig.rerunOf = selection.sourceRunId;
      }
      ...
      selectionFile.cleanup();
    }
  }
  planRerun(options, mergedConfig, playwrightArgs) {
    const scope = String(options.rerun);
    if (!isRerunScope(scope)) {
      throw new TestDinoError(`--rerun must be one of: ${RERUN_SCOPES.join(", ")} (got "${scope}").`);
    }
    if (!options.fromRun) {
      throw new TestDinoError("--rerun requires --from-run <runId>: the run whose failures should be re-executed.");
    }
    if (!mergedConfig.token) {
      throw new TestDinoError("--rerun needs a TestDino token (--token or TESTDINO_TOKEN).");
    }
    if (mergedConfig.split !== void 0 || mergedConfig.splitId !== void 0) {
      throw new RerunSplitConflictError(mergedConfig.split !== void 0 ? "--split" : "--split-id");
    }
    const conflict = conflictingSelector(playwrightArgs);
    if (conflict) {
      throw new TestDinoError(
        `--rerun cannot be combined with ${conflict}: the re-run selection already decides which tests run.`
      );
    }
    if (hasShardFlag(playwrightArgs)) {
      throw new TestDinoError(
        "--rerun cannot be combined with --shard: a re-run is a handful of tests, so every other shard would find nothing and fail."
      );
    }
    const playwrightVersion = readInstalledPlaywrightVersion();
    if (!supportsTestList(playwrightVersion)) {
      throw new TestDinoError(
        `--rerun needs Playwright 1.56 or newer for --test-list (found ${playwrightVersion ?? "no readable version"}). On older releases the selection cannot be passed safely and the whole suite would run instead.`
      );
    }
    return {
      scope,
      ...
      testIds: splitIds(options.testIds),
      excludeTestIds: splitIds(options.excludeIds)
    };
  }
  // null means "nothing to re-run" — reported, not an error.
  async resolveRerunSelection(options, mergedConfig, playwrightArgs) {
    const plan = this.planRerun(options, mergedConfig, playwrightArgs);
    const selection = await fetchRerunSelection({
      serverUrl: mergedConfig.serverUrl ?? DEFAULT_SERVER_URL,
      token: plan.token,
      runId: plan.fromRun,
      scope: plan.scope,
      testIds: plan.testIds,
      excludeTestIds: plan.excludeTestIds
    });
    const { selected, inScope, missingTestIds, unrenderableTests } = selection.resolution;
    const what = plan.testIds ? "selected" : SCOPE_LABELS[plan.scope];
    for (const test of unrenderableTests ?? []) {
      this.logger.warn(
        `TestDino re-run: "${test.title}" cannot be selected by title (it contains " \u203A "); run it by hand.`
      );
    }
    if (missingTestIds?.length) {
      this.logger.warn(
        `TestDino re-run: ${missingTestIds.length} requested test id(s) are not in run ${plan.fromRun}.`
      );
    }
    if (selected === 0) {
      this.logger.info(`TestDino re-run: nothing to re-run \u2014 run ${plan.fromRun} has no ${what} tests in scope.`);
      return null;
    }
    this.logger.info(`TestDino re-run: ${selected} of ${inScope} ${what} test(s) from run ${plan.fromRun}`);
    return selection;
  }
  // Playwright runs whatever matched and exits 0 even when nothing did, so a
  // zero match must be refused here or a stale selection reports green.
  async verifyRerunSelection(selectionPath, playwrightArgs, selection) {
    const selected = selection.testList.length;
    const match = await countSelectedTests(selectionPath, playwrightArgs);
    if (!match.checked) {
      this.logger.warn(
        `TestDino re-run: could not verify the selection against this checkout (${match.reason ?? "unknown"}); running it as-is.`
      );
      return;
    }
    if (match.matched === selected) return;
    if (match.matched === 0) {
      throw new TestDinoError(
        `None of the ${selected} selected test(s) exist in this checkout: renamed, moved or deleted since run ${selection.sourceRunId}, or excluded by the Playwright args you passed (--project, a file filter). Nothing was run.`
      );
    }
    if (match.matched > selected) {
      throw new TestDinoError(
        `The selection matched ${match.matched} tests but ${selected} were selected, so it is broader than intended. Run the tests by hand and report run ${selection.sourceRunId} to TestDino.`
      );
    }
    this.logger.warn(
      `TestDino re-run: ${selected - match.matched} of ${selected} selected test(s) do not exist in this checkout (renamed, moved, deleted, or excluded by the Playwright args you passed) and will be skipped.`
    );
  }
};
function splitIds(csv) {
  const ids = (csv ?? "").split(",").map((id) => id.trim()).filter((id) => id !== "");
...
function buildProgram() {
  ...
  program.command("test").description("Run Playwright tests with TestDino reporter").option("-t, --token <token>", "TestDino authentication token").option("--ci-run-id <id>", "CI run ID for grouping test runs").option("--server-url <url>", "TestDino server URL").option("--debug", "Enable debug logging").option("--no-artifacts", "Disable artifact uploads (screenshots, videos, traces)").option("--coverage", "Enable code coverage collection").option("--tags <tags>", "Comma-separated run-level tags (e.g. smoke,prod)").option("--split <current/total>", "Manual split index/total (e.g. 1/5) \u2014 requires --split-id").option("--split-id <id>", "Group ID correlating all splits of one logical run").option("--rerun <scope>", "Re-run only the source run's failed, flaky or failed-and-flaky tests").option("--from-run <runId>", "The run to re-run from (required with --rerun)").option("--test-ids <ids>", "Comma-separated test ids to re-run (overrides the --rerun scope)").option("--exclude-ids <ids>", "Comma-separated test ids to drop from the --rerun scope").allowUnknownOption().allowExcessArguments().action(async (options, command) => {
    if (options.debug) {
