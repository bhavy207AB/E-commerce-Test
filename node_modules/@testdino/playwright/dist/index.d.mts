...
/** One test excluded by the compiled skip filter before the run started. */
interface ActionsSkipTest {
    title: string;
    /** Describe-chain only — no project/file segments (mirrors dashboard breadcrumbs). */
    titlePath: string[];
    file: string;
    line?: number;
    project?: string;
    tags?: string[];
    /** Ids of the skip rules whose pattern matched this test (best-effort attribution). */
    ruleIds: string[];
}
/**
 * Skip-transparency blob attached to run metadata: which skip rules were
 * applied and which tests they excluded. `enumerated: false` means the list
 * pass failed — tests is empty because we could not find out, NOT because
 * nothing matched; consumers must render the two states differently.
 */
interface ActionsSkipMetadata {
    rules: Array<{
        id: string;
        name: string;
    }>;
    tests: ActionsSkipTest[];
    total: number;
    truncated: boolean;
    enumerated: boolean;
    /**
     * True when the enumeration pass found zero surviving tests. CLI-orchestration
     * signal (drives whether `--pass-with-no-tests` gets appended); optional
     * because a failed enumeration (`enumerated: false`) never sets it.
     */
    allSkipped?: boolean;
}
...
interface PlaywrightMetadata {
    ...
    workers?: number;
    shard?: ShardMetadata;
    /** Skip-transparency blob — nested here so it rides the existing pw_data JSONB server-side (no migration). */
    actionsSkip?: ActionsSkipMetadata;
...
interface ActionTraceEntry {
    ...
    actionApplied: 'quarantine' | 'tag';
    originalOutcome: 'failed' | 'timedOut' | 'passed';
    tags?: string[];
    tagValue?: string;
}
...
declare class TestdinoReporter implements Reporter {
    ...
    private actionRules?;
    private runGitContext?;
    private actionsSkip?;
    private failedFinalTests;
    private actionApplications;
    private firedActionRuleIds;
    ...
    private splitFields;
    private parseRunGitContext;
    private buildMatchContext;
    /** Pre-test-matchable tag rules append their tags to the test:begin tags (deduped). */
    private applyPreTestTagRules;
    private applyPreTestTagRulesUnsafe;
    private evaluateActionsOnTestEnd;
    private evaluateActionsOnTestEndUnsafe;
    private quarantinedTestCount;
    private recordActionApplication;
