...
var PERMANENT_HTTP_STATUSES = /* @__PURE__ */ new Set([400, 401, 402, 403, 404, 405, 409, 413]);
...
var EventBuffer = class {
  ...
  // Resolves only when nothing is in flight AND the buffer is empty: a waiter
  // that returned on "empty" alone let onEnd send run:end while a flush started
  // by another waiter was still in flight (the server then finalized early).
  async flush() {
...
var TestdinoReporter = class _TestdinoReporter {
  ...
  // Set after run:begin is delivered when auth reports the project over its
  // execution quota: suppress all further streaming without failing the build.
  ...
  constructor(config = {}) {
    ...
    this.buffer = new EventBuffer({
      ...
      onFlush: async (events) => {
        ...
        if (this.overLimitBlocked) return;
  ...
  loadCliConfig() {
    ...
    try {
      ...
      if (typeof cliConfig.rerunOf === "string" && cliConfig.rerunOf.length > 0) {
        this.rerunOf = cliConfig.rerunOf;
      }
      return mappedConfig;
    } catch (error) {
      if (isDebugEnabled()) {
  ...
  async performAsyncInit(config, suite, token) {
    ...
    try {
      ...
      const beginEvent = {
        ...
        ...this.rerunOf && { rerunOf: this.rerunOf },
        timestamp: Date.now(),
        sequence: 0
      };
      ...
      if (this.deliveryManager) {
        ...
        if (auth.overLimit) {
          this.overLimitBlocked = true;
          this.log.warn(
            "Over execution limit \u2014 this run will not be recorded on TestDino. Tests will continue to run."
          );
          return true;
        }
  ...
  async onTestBegin(test2, result) {
    if (!this.initPromise || this.initFailed || this.overLimitBlocked) return;
  ...
  onTestEnd(test2, result) {
    ...
    if (!this.openAttemptsFinalized) {
      ...
      const isFinalAttempt = result.status === "interrupted" || test2.outcome() !== "unexpected" || result.retry === test2.retries;
      if (isFinalAttempt) {
        this.totalTests++;
  ...
  async onEnd(result) {
    ...
    const success = await this.initPromise;
    if (this.overLimitBlocked) {
      if (this.pendingTestEndPromises.size > 0) {
        await Promise.allSettled(Array.from(this.pendingTestEndPromises));
      }
      this.printOverLimitNotice();
      this.log.success("Tests completed \u2014 this run was not recorded on TestDino (over execution limit)");
      this.buffer?.destroy();
      await this.kafkaProducer?.close();
      await this.printUpdateNoticeIfAvailable();
  ...
  // Advisory, non-failing notice — deliberately worded to NOT collide with
  // printQuotaError's build-failing "Execution Limit Reached" banner, and on
  // console.warn (a heads-up, not an error). The overLimit flag carries no quota
  // detail payload (plan, reset date, usage), so there is nothing to render from.
  printOverLimitNotice() {
    const border = "\u2550".repeat(70);
    console.warn("");
    console.warn(border);
    console.warn("  \u26A0\uFE0F  TestDino: this run was not recorded");
    console.warn(border);
    console.warn("");
    console.warn("  Your organization is over its execution limit, so this run's");
    console.warn("  results were not sent to TestDino and will not appear on the");
    console.warn("  dashboard. Your tests and build status are unaffected.");
    console.warn("");
    console.warn("  Upgrade your plan or wait for the quota to reset \u2014");
    console.warn("  Settings \u2192 Billing & Usage.");
    console.warn(border);
    console.warn("");
  }
