...
var TestdinoReporter = class _TestdinoReporter {
  ...
  // so the `missing` operator stays accurate on clean passes.
  ...
  /** Pre-test-matchable tag rules append their tagValue to the test:begin tags (deduped). */
  applyPreTestTagRules(test2) {
    ...
    for (const rule of this.actionRules.rules) {
      if (rule.action !== "tag" || !rule.tagValue || !isPreTestMatchable(rule)) continue;
      if (matchRule(rule, ctx)) {
        added.push(rule.tagValue);
  ...
  evaluateActionsOnTestEnd(test2, result) {
    ...
    for (const rule of this.actionRules.rules) {
      if (rule.action === "quarantine") {
        if (isFailure && matchRule(rule, ctx)) {
          quarantined = true;
      ...
      } else if (rule.action === "tag" && rule.tagValue) {
        if (matchRule(rule, ctx)) {
          this.firedActionRuleIds.add(rule.id);
          traces.push({
            actionRuleId: rule.id,
  ...
  async onTestBegin(test2, result) {
    ...
    const event = {
      ...
      // Test configuration (Actions tag rules may append their tagValue, deduped)
