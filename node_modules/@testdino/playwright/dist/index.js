...
function ruleTags(rule) {
  return rule.action.find((entry) => entry.op === "tag")?.details?.tags ?? [];
}
function parseActionsSkipMetadata(raw) {
  if (raw === null || typeof raw !== "object") return void 0;
  const data = raw;
  if (!Array.isArray(data.rules) || !Array.isArray(data.tests)) return void 0;
  const rules = [];
  for (const entry of data.rules) {
    if (entry === null || typeof entry !== "object") return void 0;
    const { id, name } = entry;
    if (typeof id !== "string" || typeof name !== "string") return void 0;
    rules.push({ id, name });
  }
  const tests = [];
  for (const entry of data.tests) {
    if (entry === null || typeof entry !== "object") return void 0;
    const t = entry;
    if (typeof t.title !== "string" || typeof t.file !== "string") return void 0;
    if (!Array.isArray(t.titlePath) || !t.titlePath.every((s) => typeof s === "string"))
      return void 0;
    if (!Array.isArray(t.ruleIds) || !t.ruleIds.every((s) => typeof s === "string")) return void 0;
    tests.push({
      title: t.title,
      ...
      ...Array.isArray(t.tags) && t.tags.every((s) => typeof s === "string") && { tags: t.tags },
      ruleIds: t.ruleIds
    });
  }
  return {
    ...
    enumerated: data.enumerated === true,
    ...data.allSkipped === true && { allSkipped: true }
  };
}
...
function matchRule(rule, ctx) {
  return matchNode(rule.conditions, ctx);
}
function matchNode(node, ctx) {
  if (isConditionGroup(node)) return matchGroup(node, ctx);
  return matchCondition(node, ctx);
}
function matchGroup(group, ctx) {
  if (group.cond.length === 0) return false;
  switch (group.op) {
    case "AND":
      return group.cond.every((child) => matchNode(child, ctx));
    case "OR":
      return group.cond.some((child) => matchNode(child, ctx));
    default:
      return false;
  }
}
...
var MAX_LISTED_TESTS = 10;
function formatActionsSummary(records, rules) {
  const quarantined = records.filter((record) => record.quarantinedBy.length > 0);
  const tagged = records.filter((record) => record.tags.length > 0);
  const skipRules = rules.filter((rule) => ruleHasAction(rule, "skip"));
  if (quarantined.length === 0 && tagged.length === 0 && skipRules.length === 0) return [];
  const lines = ["TestDino Actions summary:"];
  if (quarantined.length > 0) {
    lines.push(`  quarantined ${quarantined.length} test(s):`);
    for (const record of quarantined.slice(0, MAX_LISTED_TESTS)) {
      lines.push(`    - ${record.title}  [${record.quarantinedBy.join(", ")}]`);
    }
    if (quarantined.length > MAX_LISTED_TESTS) {
      lines.push(`    ...and ${quarantined.length - MAX_LISTED_TESTS} more`);
    }
  }
  if (tagged.length > 0) {
    lines.push(`  tagged ${tagged.length} test(s):`);
    for (const record of tagged.slice(0, MAX_LISTED_TESTS)) {
      lines.push(`    - ${record.title}  +${record.tags.join(" +")}  [${record.taggedBy.join(", ")}]`);
    }
    if (tagged.length > MAX_LISTED_TESTS) {
      lines.push(`    ...and ${tagged.length - MAX_LISTED_TESTS} more`);
    }
  }
  if (skipRules.length > 0) {
    lines.push(
      `  skip rule(s) in ruleset: ${skipRules.map((rule) => rule.name).join(", ")} (applied before the run via --grep-invert; skipped tests do not appear in the output above)`
    );
  }
  return lines;
}
...
var TestdinoReporter = class _TestdinoReporter {
  ...
  splitId;
  runStartTime;
  // TestDino Actions — ruleset + git context ride the CLI temp-config, stored outside this.config like splitInfo.
  actionRules;
  runGitContext;
  actionsSkip;
  failedFinalTests = /* @__PURE__ */ new Map();
  actionApplications = /* @__PURE__ */ new Map();
  firedActionRuleIds = /* @__PURE__ */ new Set();
  sigintHandler;
  sigtermHandler;
  ...
  loadCliConfig() {
    ...
    try {
      ...
      if (cliConfig.actionRules !== void 0) {
        this.actionRules = parseActionRuleset(cliConfig.actionRules) ?? void 0;
      }
      this.runGitContext = this.parseRunGitContext(cliConfig.runGitContext);
      this.actionsSkip = parseActionsSkipMetadata(cliConfig.actionsSkip);
      return mappedConfig;
    } catch (error) {
      if (isDebugEnabled()) {
        console.warn(
  ...
  parseRunGitContext(raw) {
    if (raw === null || typeof raw !== "object") return void 0;
    const data = raw;
    const git = {};
    if (typeof data.branch === "string") git.branch = data.branch;
    if (typeof data.authorEmail === "string") git.authorEmail = data.authorEmail;
    if (typeof data.authorName === "string") git.authorName = data.authorName;
    if (typeof data.message === "string") git.message = data.message;
    if (typeof data.remote === "string") git.remote = data.remote;
    return git;
  }
  // errorMessage exists only post-test AND only when the attempt actually errored,
  // so the value-list `missing` operators stay accurate on clean passes.
  // annotations are the statically-declared annotation TYPE strings (a test
  // with none resolves to [], so `isEmpty` reads true rather than absent).
  buildMatchContext(test2, result) {
    return {
      ...
      testId: test2.id,
      tags: test2.tags,
      annotations: (result?.annotations ?? test2.annotations).map((a) => a.type),
      project: this.getProjectName(test2),
      ...this.runGitContext && { git: this.runGitContext },
      ...result && result.errors.length > 0 && {
        errorMessage: result.errors.map((e) => e.message ?? "").join("\n")
      }
    };
  }
  /** Pre-test-matchable tag rules append their tags to the test:begin tags (deduped). */
  applyPreTestTagRules(test2) {
    if (!this.actionRules || this.actionRules.rules.length === 0) return test2.tags;
    try {
      return this.applyPreTestTagRulesUnsafe(test2);
    } catch (error) {
      this.log.warn(`TestDino Actions: tag-rule matching failed unexpectedly, skipping for this test (${error})`);
      return test2.tags;
    }
  }
  applyPreTestTagRulesUnsafe(test2) {
    if (!this.actionRules) return test2.tags;
    const ctx = this.buildMatchContext(test2);
    const added = [];
    for (const rule of this.actionRules.rules) {
      const tags = ruleTags(rule);
      if (tags.length === 0 || !isPreTestMatchable(rule)) continue;
      if (matchRule(rule, ctx)) {
        added.push(...tags);
        this.firedActionRuleIds.add(rule.id);
      }
    }
    return added.length > 0 ? [.../* @__PURE__ */ new Set([...test2.tags, ...added])] : test2.tags;
  }
  // Final attempts only. Records every failed final test (matched or not) so onEnd
  // can neutralize the exit code ONLY when all failures are quarantine-matched.
  evaluateActionsOnTestEnd(test2, result) {
    if (!this.actionRules || this.actionRules.rules.length === 0) return void 0;
    try {
      return this.evaluateActionsOnTestEndUnsafe(test2, result);
    } catch (error) {
      this.log.warn(`TestDino Actions: rule matching failed unexpectedly for a test, skipping (${error})`);
      return void 0;
    }
  }
  evaluateActionsOnTestEndUnsafe(test2, result) {
    if (!this.actionRules) return void 0;
    const outcome = result.status;
    if (outcome !== "failed" && outcome !== "timedOut" && outcome !== "passed") return void 0;
    const isFailure = outcome === "failed" || outcome === "timedOut";
    const ctx = this.buildMatchContext(test2, result);
    const traces = [];
    let quarantined = false;
    const ruleIds = [];
    for (const rule of this.actionRules.rules) {
      const hasQuarantine = ruleHasAction(rule, "quarantine");
      const tags = ruleTags(rule);
      const needsMatch = tags.length > 0 || hasQuarantine && isFailure;
      if (!needsMatch || !matchRule(rule, ctx)) continue;
      this.firedActionRuleIds.add(rule.id);
      if (hasQuarantine && isFailure) {
        quarantined = true;
        ruleIds.push(rule.id);
        traces.push({
          actionRuleId: rule.id,
          actionRuleName: rule.name,
          actionApplied: "quarantine",
          originalOutcome: outcome
        });
        this.recordActionApplication(test2, rule.name, "quarantine");
      }
      if (tags.length > 0) {
        traces.push({
          actionRuleId: rule.id,
        ...
        this.recordActionApplication(test2, rule.name, "tag", tags);
      }
    }
    if (isFailure) {
      this.failedFinalTests.set(test2.id, { quarantined, ruleIds });
    }
    return traces.length > 0 ? traces : void 0;
  }
  quarantinedTestCount() {
    let count = 0;
    for (const entry of this.failedFinalTests.values()) {
      if (entry.quarantined) count++;
    }
    return count;
  }
  // Keyed by test id so a retried final attempt merges instead of duplicating.
  recordActionApplication(test2, ruleName, action, tags) {
    const record = this.actionApplications.get(test2.id) ?? {
      title: test2.title,
      quarantinedBy: [],
      tags: [],
      taggedBy: []
    };
    if (action === "quarantine" && !record.quarantinedBy.includes(ruleName)) {
      record.quarantinedBy.push(ruleName);
    }
    if (action === "tag") {
      if (!record.taggedBy.includes(ruleName)) record.taggedBy.push(ruleName);
      for (const tag of tags ?? []) {
        if (!record.tags.includes(tag)) record.tags.push(tag);
      }
    }
    this.actionApplications.set(test2.id, record);
  }
  ...
  async performAsyncInit(config, suite, token) {
    ...
    try {
      ...
      if (this.actionsSkip) {
        metadata.playwright = { ...metadata.playwright, actionsSkip: this.actionsSkip };
      }
  ...
  async onTestBegin(test2, result) {
    ...
    const event = {
      ...
      // Test configuration (Actions tag rules may append their tags, deduped)
      tags: this.applyPreTestTagRules(test2),
  ...
  onTestEnd(test2, result) {
    ...
    if (!this.openAttemptsFinalized) {
      ...
      if (isFinalAttempt) {
        ...
        actionTraces = this.evaluateActionsOnTestEnd(test2, result);
      }
    }
    const workPromise = this.processTestEnd(test2, result, actionTraces);
  ...
  async processTestEnd(test2, result, actionTraces) {
    try {
  ...
  async onEnd(result) {
    ...
    const event = {
      ...
      ...this.actionRules && this.firedActionRuleIds.size > 0 && {
        actions: {
          quarantinedCount: this.quarantinedTestCount(),
          appliedActionRuleIds: [...this.firedActionRuleIds]
        }
      }
    };
    this.buffer?.destroy();
    ...
    if (this.actionRules) {
      for (const line of formatActionsSummary([...this.actionApplications.values()], this.actionRules.rules)) {
        this.log.info(line);
      }
    }
    ...
    if (result.status === "failed" && this.failedFinalTests.size >= 1 && !coverageFailed && !deliveryFailed && [...this.failedFinalTests.values()].every((entry) => entry.quarantined)) {
      this.log.info(
        `${this.failedFinalTests.size} failed test(s) quarantined by TestDino Actions - exit code neutralized`
      );
      if (!this.isShuttingDown) {
        await this.kafkaProducer?.close();
        await this.printUpdateNoticeIfAvailable();
